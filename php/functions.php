...
function get_navigation($option) {
	...
	if ($option === 'menu') {
		...
		if ($_SERVER['HTTP_HOST'] === 'localhost') {
			$homepath = '/seccalendar.co.uk/trunk/';
		} else {
			$homepath = '/';
		}
		echo($homepath);
...
class CNavigation {
	public static function GenerateMenu($items) {
		...
		$remoteRoot = '/';
		$localRoot  = '/seccalendar.co.uk/trunk';
		...
		// echo("<h1>$isHomePage</h1>");

		// Output HOME menu item
		if ($location === 'remote') {
		    $homepath = '/';
		} else {
		    $homepath = '/seccalendar.co.uk/trunk/';
		}
		$html = "<li class='$homepathClass'><a href='$homepath'>Home</a></li>";

		// Output OTHER menu items
